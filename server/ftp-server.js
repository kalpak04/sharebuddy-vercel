...
const getPublicIp = async () => {
  try {
    ...
    if (process.env.PASV_URL || process.env.PUBLIC_IP) {
      return process.env.PASV_URL || process.env.PUBLIC_IP;
    }
...
// Initialize FTP server with better configuration
const initFtpServer = async () => {
  ...
  return new FtpSrv({
    ...
    anonymous: false,
    pasv_url: publicIp,
    pasv_min: parseInt(process.env.PASV_MIN, 10) || 30000, // Higher port range for Render
    pasv_max: parseInt(process.env.PASV_MAX, 10) || 50000,
...
let ftpServer;
(async () => {
  ftpServer = await initFtpServer();
  setupEventHandlers(ftpServer);
})();

// Separate event handler setup
const setupEventHandlers = (server) => {
  ...
  server.on('STOR', async ({ connection, file }) => {
  ...
  server.on('client-error', ({ connection, context, error }) => {
    ...
    log.error('Client error', { error, context, client: clientInfo });
    
    // Attempt to gracefully close connection if still open
...
process.on('SIGTERM', async () => {
  try {
    if (ftpServer) {
      ...
      log.info('FTP server closed gracefully');
    }
  } catch (err) {
